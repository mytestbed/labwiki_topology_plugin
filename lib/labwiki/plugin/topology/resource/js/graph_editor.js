  ...
  var graph_editor = function(container, opts) {
    ...
    function restart_links() {
      ...
      pe.append('line')
          ...
          .on('mouseover', function(d) {
            ...
            show_tooltip(d, d3.select(this), 'link');
          })
    ...
    function element_event_handling(selection) {
      selection
        .on('mouseover', function(d) {
          // enlarge target node
          d3.select(this).attr('transform', 'scale(' + hover_scale + ')');
          if (d.type == 'node') {
          ...
          show_tooltip(d, d3.select(this), 'element');
        })
        .on('mouseout', function(d) {
    ...
    function show_interfaces(links, target_selected) {
      ...
      interfaces_g.selectAll('.interface').data(ifs).enter().append('svg:circle')
        ...
        .on('mouseover', function(d) {
          var x = 0;
          show_tooltip(d, d3.select(this), 'interface');
        })
        .on('mouseout', function(d) {
          hide_tooltip();
        })
