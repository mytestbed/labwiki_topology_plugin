...
define(['theme/labwiki/js/labwiki', 'plugin/topology/js/graph_editor', 'theme/labwiki/js/form_builder'],
  function (LW, GraphEditor, FormBuilder) {
    ...
    var topology_editor = function(container, controller, opts) {
      var editable = opts.editable || true;
      var topology_name = opts.topology_name;
      var toolbar_buttons = {};
      var selected_element = null;

      var info_panel_height = 200;

      var topo_descr = opts.topology || {graph: {}};
      if (typeof topo_descr == 'string') topo_descr = JSON.parse(topo_descr);

      var aggregates = _.map(opts.aggregates || [], function(a) {
        return {label: a.name, value: a.urn}
      });
      aggregates.unshift({label: '---', value: null});

      var defs = topo_descr.graph.defaults || {};
      var node_defs = defs.modes || {};
      var sliver_types = _.map(['emulab-xen'], function (st) {return {value: st};});
      var parameters = {
        node: {
          name: { label: 'Name', type: 'text' },
          am_urn: {
            label: 'AM', optional: true, type: 'selector',
            choices: aggregates,
            def_value: node_defs.am_urn || aggregates[1].value
          },
          sliver_type: {
            label: 'Sliver Type', type: 'selector',
            choices: sliver_types,
            def_value: sliver_types[0].value
          },
      ...
      var graph_editor = GraphEditor(topo_descr, container.find('.topology_editor'), {
                            id: graph_editor_id,
                            editable: editable,
      ...
      //
      function graph() {
        var graph = graph_editor.state();
        var defs = graph.graph.defaults = {};
      ...
      function resize() {
        ...
        var height = Math.floor(controller.get('panel_height') - info_panel_height);

        graph_editor.resize(width, height);
      }
      OHUB.bind('column.prepare.resize', resize);
      ...
      // Add a checkbox to mark some of the entries as defaults for all otehr entities of the same type
      ...
      b.save = pc.add_toolbar_button({name: 'save', awsome: 'floppy-o', tooltip: 'Save', active: true},
        function() {
          ...
          controller.request_action(opts, 'POST', function(reply) {
            controller.show_alert('info', 'Successfully saved');
          });
          return false;
        });
